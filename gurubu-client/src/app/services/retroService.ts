...
export class RetroService {
  ...
  async getRetroActionItems(retroId: string) {
    try {
      const response = await axios.get(`${this.baseUrl}/retro/${retroId}/action-items`);
      return response.data;
    } catch (error) {
  ...
  async checkRetroBatch(retroIds: string[]): Promise<{
    ...
    try {
      ...
      return { existingIds: [], participants: {}, statuses: {} };
    }
  }
}
